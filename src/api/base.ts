...
export class BaseAPI {
    ...
    /**
     * Завершает соединение, уничтожая агент
     */
    async close() {
    ...
    protected async request(type:'GET'|'POST'|'PUT'|'DELETE', route:string, body?:FormData|object, callback?: (res?:string)=>object|undefined, extraHeaders?:object ): Promise<ResponseSchema> {
        ...
        switch(type) {
            ...
            case 'POST':
                ...
                const isFormData = body && typeof (body as any).getBoundary === 'function';
                const content_type = isFormData ? undefined : {'Content-Type': 'application/json'};
                const raw_body: FormData | string = isFormData ? (body as FormData) : JSON.stringify({
